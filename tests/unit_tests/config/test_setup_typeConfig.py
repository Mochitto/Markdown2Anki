...
class TestTypeConfig:
    ...
    def test_options_and_types(self, tmp_dirs):
        ...
        config_options = ['Obsidian valut name', 'search images folder', 'images out-folder', 'bad cards file path', 'input md file path', 'Number of backups', 'clear file?', 'line numbers?', 'fast forward?', 'folders to exclude']
        ...
        options_message = f"Config lacks option/s {lacking_options} " if lacking_options else ''
        options_message += f"Config has extra option/s {extra_options}" if extra_options else ''
        assert not options_message

    def test_types(self, tmp_dirs):
        config = setup_typeConfig(tmp_dirs["config"])

        config_types = ['ExistingPath', 'AbsolutePath', 'NewFolder', 'FoldersList', 'bool', 'str', 'int'] 
